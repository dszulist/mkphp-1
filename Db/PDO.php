...
class MK_Db_PDO
{
    ...
    /**
     * Zapamiętanie właściwości, żeby nie odczytywać dwa razy z bazy danych
     *
     * @var array
     */
     ...
     * @return array
     */
    public function getCompletedTask($limit = 10, $forceRefresh = false)
    {
        if($forceRefresh === true || !isset($this->propertyCache[__METHOD__])) {
            $this->propertyCache[__METHOD__] = $this->GetRows('SELECT * FROM upgrade_completed_task ORDER BY id DESC LIMIT ?', array ($limit));
        }
        return $this->propertyCache[__METHOD__];
    }
     ...
     * @return array
     */
    public function getCountTablesRow($forceRefresh = false)
    {
        if($forceRefresh === true || !isset($this->propertyCache[__METHOD__])) {
            $counter = array ();
            ...
            $this->propertyCache[__METHOD__] = $counter;
        }
        return $this->propertyCache[__METHOD__];
    }
