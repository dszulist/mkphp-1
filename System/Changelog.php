...
BEGIN
	...
	IF version_id IS NOT NULL THEN
		SELECT INTO desc_exists description FROM system_changelog WHERE description = $2 AND system_version_id = version_id;
		IF NOT FOUND THEN
			EXECUTE 'INSERT INTO system_changelog(system_version_id, description, description_more, patch_type, bt_id) VALUES ('||version_id||','|| quote_literal($2) ||', '|| quote_literal($3) ||', '''|| $4 ||''','|| quote_literal($5) ||');';
		ELSE
			EXECUTE 'UPDATE system_changelog SET description_more = '|| quote_literal($3) || ' WHERE description = '|| quote_literal($2) || ' AND system_version_id = '||version_id;
		END IF;
	ELSE
