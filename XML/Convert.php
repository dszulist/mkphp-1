...
class MK_XML_Convert {
    ...
    private function getXMLReader(){
        ...
        // zgodnosc z debian lenny (libxml2 jest w wersji 2.6.32.dfsg-5+lenny5, a define do duzych xmli dziala od wersji libxml2.7.3)
        if(defined('LIBXML_PARSEHUGE')){
            $xmlReader->$method($this->xmlSrc, NULL, LIBXML_PARSEHUGE);
        }
        else{
    ...
    public function getAsObject($xml=null){
        ...
        // jesli aplikacja korzysta z __autoload nalezy przywrocic jego obsluge
        // (http://www.php.net/manual/pl/function.spl-autoload-unregister.php#92062)
    ...
    final public static function autoloadProxyClass($className){
        // w trakice tego autoload'a nie dziala fkcja __autoload
        $mainClass = 'MK_XML_ProxyClassAbstract';
        $mainClassPath = str_replace('_',DIRECTORY_SEPARATOR,$mainClass) . '.php';
        require_once $mainClassPath;
        eval("Final Class {$className} extends {$mainClass} { }");
        return true;
    }
