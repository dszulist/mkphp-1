...
Class MK_Upgrade extends MK_Db_PDO
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $this->exceptionLog("MK_Db_Exception: {$e->getExtendedMessage()}, Kod: {$e->getCode()}, Plik: {$e->getFile()}, Linia: {$e->getLine()}");
        } catch (MK_Exception $e) {
            $this->exceptionLog("MK_Exception: {$e->getExtendedMessage()}, Kod: {$e->getCode()}, Plik: {$e->getFile()}, Linia: {$e->getLine()}");
        } catch (Exception $e) {
            $this->exceptionLog("Exception: {$e->getMessage()}, Kod: {$e->getCode()}, Plik: {$e->getFile()}, Linia: {$e->getLine()}");
        }
    }

    /**
     * Zwrócenie wyjątku i zapisanie informacji w logach
     *
     * @param $exceptionMsg
     */
    public function exceptionLog($exceptionMsg)
    {
        self::writeToLog('UPGRADE PRZERWANY!!!');
        self::writeToLog($exceptionMsg);
